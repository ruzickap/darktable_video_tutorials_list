...
echo $YOUTUBE_VIDEO_LENGTH | grep "^[0-9]:" && YOUTUBE_VIDEO_LENGTH="0${YOUTUBE_VIDEO_LENGTH}"
...
shift
while [[ $# -gt 0 ]]; do
  echo $1 | grep "^[0-9]:" && TIME="0$1" || TIME="$1"
  echo "![${YOUTUBE_VIDEO_NAME}](screenshots/${YOUTUBE_VIDEO_NAME_SCREENSHOT}-${TIME}.jpg \"${YOUTUBE_VIDEO_NAME} (${TIME})\")"
