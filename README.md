...
```
YOUTUBE_VIDEO_ID="SKknBy5lX7I"
YOUTUBE_VIDEO_NAME=`youtube-dl --get-title https://youtu.be/${YOUTUBE_VIDEO_ID}`
YOUTUBE_VIDEO_NAME_SCREENSHOT=`echo ${YOUTUBE_VIDEO_NAME} | sed 's/ /_/g;s/#//;s/|/-/'`
YOUTUBE_VIDEO_LENGTH=`youtube-dl --get-duration https://youtu.be/${YOUTUBE_VIDEO_ID}`
YOUTUBE_VIDEO_RESOLUTION=`youtube-dl --get-format https://youtu.be/${YOUTUBE_VIDEO_ID} | awk '{ print $3 }'`
...
![${YOUTUBE_VIDEO_NAME}](screenshots/${YOUTUBE_VIDEO_NAME_SCREENSHOT}-xx:xx.jpg "${YOUTUBE_VIDEO_NAME} (xx:xx)")
EOF

mpv --screenshot-template="${YOUTUBE_VIDEO_NAME_SCREENSHOT}-%wM:%wS" --screenshot-jpeg-quality=70 https://youtu.be/${YOUTUBE_VIDEO_ID}
```
